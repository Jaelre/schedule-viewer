...
(function (ha) {
	...
	function Z(b, c, d, k) {
		c &&
			E(c, function (c, g) {
				if (d || !w(b, g)) !k || 'object' !== typeof c || !c || M(c) || L(c) || c instanceof RegExp ? (b[g] = c) : (b[g] || (b[g] = {}), Z(b[g], c, d, k));
			});
		return b;
	}
	function y(b, c) {
	...
	function ma(b) {
		function c(a, n, b) {
			...
			if (a) {
				...
				for (c = 0; c < l.length; c++)
					(d = l[c]), '.' === d ? (l.splice(c, 1), --c) : '..' === d && 0 !== c && (1 !== c || '..' !== l[2]) && '..' !== l[c - 1] && 0 < c && (l.splice(c - 1, 2), (c -= 2));
				a = a.join('/');
			}
			if (b && q && (n || p)) {
				l = a.split('/');
		...
		function I() {
			...
			if (!ba) {
				...
				if (c && f.length) return (a = G('timeout', 'Load timeout for modules: ' + f, null, f)), (a.contextName = h.contextName), B(a);
				g &&
					x(l, function (a) {
						K(a, {}, {});
					});
							...
							if (L(l)) {
								...
								if (a) {
									if ((this.events.error && this.map.isDefine) || k.onError !== ia)
										return (a.requireMap = this.map), (a.requireModules = this.map.isDefine ? [this.map.id] : null), (a.requireType = this.map.isDefine ? 'define' : 'require'), B((this.error = a));
		...
		F &&
			...
			Y(document.getElementsByTagName('script'), function (b) {
				...
				if ((P = b.getAttribute('data-main')))
					return (
						(u = P),
						v.baseUrl || ((J = u.split('/')), (u = J.pop()), (U = J.length ? J.join('/') + '/' : './'), (v.baseUrl = U)),
						(u = u.replace(V, '')),
