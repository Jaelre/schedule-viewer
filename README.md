...
- Deterministic, accessible colour palette with overrides in `src/config/shift-colors.json`.
- Configurable dictionary for shift labels via `src/config/shift-display.config.json` and doctor name mapping via `src/config/doctor-names.json`.
...
- **Styling issues after colour changes**: ensure HSL values in `src/config/shift-colors.json` maintain sufficient contrast; the fallback generator covers undefined codes.
